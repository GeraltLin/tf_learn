...
h_conv1 = tf.layers.conv2d(x_image, 64, [5, 1], 1, 'SAME', activation=tf.nn.relu)
h_conv1 = tf.layers.conv2d(h_conv1, 64, [1, 5], 1, 'SAME', activation=tf.nn.relu)

h_pool1 = tf.layers.max_pooling2d(h_conv1, [2, 2], strides=2, padding='SAME')

h_conv2 = tf.layers.conv2d(h_pool1, 64, [5,1], 1, 'SAME', activation=tf.nn.relu)
h_conv2 = tf.layers.conv2d(h_conv2, 64, [1,5], 1, 'SAME', activation=tf.nn.relu)
